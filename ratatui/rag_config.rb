...
module RAGComponents
    ...
    in {type: :key, code: 'enter'}
      ...
      Thread.new do 
        begin
          case @rag_focused
          when 0
            if File.exist?(@rag_components.fetch('rag_db'))
              @rag_process_error_msg = "RAG DB already exists."
              @rag_process_error = true;
            else 
              api_tokens = File.readlines(@rag_components['api_token_path']).map(&:strip)
              err = Setup.init_rag_db!(@rag_components, api_tokens)
              @rag_process_error_msg = err unless err.nil?
            end 
          when 1
            @rag_process_error_msg = "DB doesn't currently exist." if !File.exist?(@rag_components['rag_db'])
            FileUtils.rm_rf(Setup::RAG_DB_FILEPATH) if @rag_process_error_msg.nil?
          else  
            raise 'unknown index found.'
          end
          @rag_process_error = !@rag_process_error_msg.nil?
          @rag_process_success = !@rag_process_error
        rescue StandardError => e 
          @rag_process_error_msg = e
          @rag_process_error = true
          @rag_process_success = false
        ensure 
          @rag_process_running = false
  ...
  def render_rag_form(frame, tui, area)
    ...
    current_state = ['Current variables of interest (change if wrong): '] + @rag_components.map do |k, v| 
